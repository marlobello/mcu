...
@description('Discord guild whose members may access MCU.')
param discordGuildId string
@description('Custom hostname to configure after its DNS CNAME exists.')
param customDomain string = 'mcu.dotheneedful.dev'
@description('Set true only after the DNS CNAME points to the Static Web App hostname.')
...
module web './app/staticwebapp.bicep' = {
  ...
  scope: rg
  params: {
    name: 'stapp-mcu-${resourceToken}'
    location: location
    tags: tags
    customDomain: customDomain
    configureCustomDomain: configureCustomDomain
  }
}
...
module keyVault './app/keyvault.bicep' = {
  ...
  scope: rg
  params: {
    name: 'kv-mcu-${take(resourceToken, 12)}'
    location: location
    tags: tags
...
var storageEndpointConfig = {
  ...
  enableTable: true  // MCU application data is stored in Azure Table Storage
...
output SERVICE_API_URI string = functionAppUrl
output VITE_API_URL string = '${functionAppUrl}/api'
output SERVICE_WEB_URI string = web.outputs.url
output STATIC_WEB_APP_HOSTNAME string = web.outputs.hostname
